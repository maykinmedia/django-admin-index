...
def _build_tree(
    ...
    A group whose parent is not among the given groups, because the caller filtered
    it out, is treated as a top-level group rather than dropped. The same fallback
    catches a group that is its own parent or nested more than two levels deep,
    which validation prevents but a raw write can still produce.
    """
...
class AppGroup(TranslationsMixin, OrderedModel):
    """
    A group of admin models and links, shown as one block on the admin index page.

    Only two levels are supported: a group may have a parent, but a group that has
    a parent cannot have children of its own. validate_parent() enforces that from
    clean(), so the admin reports a field error, and limit_choices_to keeps child
    groups out of the parent dropdown to begin with. Writes that never build a
    model instance, such as loaddata and QuerySet.update(), are not covered; the
    admin_index.W001 system check reports the groups they leave behind.

    Order is relative to the parent: top-level groups are ordered among themselves,
    child groups among their siblings.
    """
    ...
    def validate_parent(self):
        """Raise a ValidationError if this group's parent breaks the nesting rule."""
        ...
        if self.parent.parent_id is not None:
            raise ValidationError(
                {
                    "parent": _(
                        "Only two levels are supported: the selected parent is "
                        "itself nested under another group."
                    )
                }
            )
