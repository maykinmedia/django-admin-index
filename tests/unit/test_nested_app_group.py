...
class NestedAppGroupCheckTests(TestCase):
    """
    Tests for the admin_index.W001 check: reporting groups that writes skipping
    validation -- loaddata, QuerySet.update() -- managed to nest too deeply.
    """
    ...
    def test_self_parenting_is_reported(self):
        ...
        result = check_app_group_nesting([])

        self.assertEqual(len(result), 1)
        self.assertIn("Parent", result[0].msg)
    ...
    def test_unavailable_table_is_not_reported_as_a_problem(self):
        ...
        with mock.patch.object(
            AppGroup.objects, "filter", side_effect=DatabaseError("no such table")
        ):
            self.assertEqual(check_app_group_nesting([]), [])

    def test_all_offending_groups_are_named(self):
        ...
        self.assertEqual(len(result), 1)
        self.assertIn("Aardvark, Zebra", result[0].msg)
    ...
    def test_over_nested_fixture_is_reported(self):
        ...
        # the fixture nests Configuration > Accounts > Permissions
        self.assertIn("Permissions", issues[0].msg)
        self.assertEqual(issues[0].id, "admin_index.W001")
    ...
    def test_admin_nesting_success(self):
        response = self.client.post(
            reverse("admin:admin_index_appgroup_add"),
            {
                "name": "Second child",
                "slug": "second-child",
                # the form field treats {} as empty, so send an actual translation
                "translations": '{"nl": "Tweede kind"}',
